<script lang="ts">
  ...
  import { bids, totalBids } from '../sku-auction.store';
  import NoAuction from './NoAuction.svelte';
  import { hasAuction } from '../sku-auction.service';
...
{#if listing?.status === 'active'}
  <div class:opacity-40={$loadingBids}>
    {#if $totalBids > 0}
          ...
          {#each $bids as bid}
            ...
            >
              ...
              <div class="justify-self-end">
                Bid for <span class="text-white px-1">{formatCurrency(bid.bidAmt, { currency: sku.currency })}</span>
              </div>
      ...
      <Pagination perPage={5} total={$totalBids} page={bidsPage} class="mt-4 flex justify-end" on:change={gotoPage} />
    {:else if $totalBids === 0 && !$loadingBids}
      <div class="no-results">No bids found</div>
    {/if}
  </div>
  <div class="text-center text-gray-500 py-6">
    Started at <span class="text-white">{formatCurrency(listing.minBid, { currency: sku.currency })}</span>
    on
